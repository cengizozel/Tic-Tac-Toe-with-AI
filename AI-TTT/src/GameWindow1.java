...
public class GameWindow1 extends JPanel implements ActionListener {
    ...
    public int randomness(int percentage, int correctMove) {
    	
    	int randomPlay = r.nextInt(9);
    	
    	while(buttons[randomPlay].getText() != "") {
    		randomPlay = r.nextInt(9);
    	}
    ...
    public int difficulty(int cm) {
    	
    	if(MainMenu.dif == "random") {
    		return randomness(0, cm);
    	}
    	else if(MainMenu.dif == "easy") {
    		return randomness(25, cm);
    	}
    	else if(MainMenu.dif == "medium") {
    		return randomness(50, cm);
    	}
    	else if(MainMenu.dif == "hard") {
    		return randomness(75, cm);
    	}
    	else if(MainMenu.dif == "impossible") {
    		return cm;
    	}
			...
			if(e.getSource()==buttons[i]) {
				if(board.getManTurn()) {
                	...
                	if (board.squarePlayable(columnChoice)) {
                        ...
                        check();
	            		if(!board.isTerminal()) {
    	            		aiMove = difficulty(ai.decideTurn(board));
    	        			aiPlay(aiMove);
    	            	}
                    }
	            }
